...
const RISK_ICONS: Record<RiskLevel, string> = {
  ...
  medium: ICONS.BADGE,
  high: ICONS.LOCK,
  critical: ICONS.SHIELD,
};

const products: Product[] = [
  { id: 'p1', name: 'AWS Credits ($100)', price: 95, icon: RISK_ICONS.low, recipient: 'So11...AWS', risk: 'low', note: 'Treasury essentials' },
  { id: 'p2', name: 'DevOps Hour', price: 150, icon: RISK_ICONS.medium, recipient: 'So11...DEV', risk: 'medium', note: 'Ops maintenance' },
  { id: 'p3', name: 'VPN Subscription', price: 12, icon: RISK_ICONS.medium, recipient: 'So11...VPN', risk: 'medium' },
  { id: 'p4', name: 'Dark Web Data', price: 499, icon: RISK_ICONS.critical, recipient: 'BAD...ADDR', risk: 'critical' }, // TRIGGER
  { id: 'p5', name: 'Quantum Farm', price: 50000, icon: RISK_ICONS.high, recipient: 'So11...QTM', risk: 'high' },
];
...
const el = {
  ...
  listServiceModal: document.getElementById('list-service-modal'),
  listServiceForm: document.getElementById('list-service-form'),
  listServiceCancel: document.getElementById('btn-cancel-list'),
  // Control Plane
    ...
    card.innerHTML = `
        ...
        <div class="product-info">
            <div class="product-meta">
                <h3>${product.name}</h3>
                <span class="product-risk">${product.risk.toUpperCase()} RISK</span>
            </div>
            <div class="product-price">$${product.price.toFixed(2)}</div>
            <div class="product-recipient">${product.recipient}</div>
            ${product.note ? `<p class="product-note">${product.note}</p>` : ''}
        </div>
        <button class="btn-full" id="btn-buy-${product.id}">BUY NOW</button>
    `;
    grid.appendChild(card);
    document.getElementById(`btn-buy-${product.id}`)?.addEventListener('click', () => handleBuy(product));
  });
}

function showListServiceModal() {
  el.listServiceModal?.classList.remove('hidden');
}

function hideListServiceModal() {
  el.listServiceModal?.classList.add('hidden');
  el.listServiceForm?.reset();
}

async function handleListServiceSubmit(event: SubmitEvent) {
  event.preventDefault();
  if (!el.listServiceForm) return;
  const data = new FormData(el.listServiceForm);
  const name = (data.get('service_name') ?? '').toString().trim();
  const price = Number(data.get('service_price'));
  const risk = (data.get('service_risk') ?? 'low') as RiskLevel;
  const recipient = (data.get('service_recipient') ?? '').toString().trim() || name;
  const note = (data.get('service_note') ?? '').toString().trim();

  if (!name || Number.isNaN(price) || price <= 0) {
    alert('Please provide a valid service name and price.');
    return;
  }
  ...
  const service: Product = {
    ...
    name,
    price,
    icon: RISK_ICONS[risk] ?? ICONS.CLOUD,
    recipient,
    risk,
    note: note || undefined
  };

  products.unshift(service);
  renderProducts();
  hideListServiceModal();
  void logThought('OPS', `Service "${name}" listed at $${price.toFixed(2)} USD1.`, 'info');

  if (risk === 'high' || risk === 'critical') {
    triggerLockdown(service, 'Governance review required for a high/critical risk listing.');
  }
}
...
function triggerLockdown(product: any, reason?: string) {
  ...
  state.governanceRequests.unshift({
    ...
    target: product.recipient ?? product.name ?? 'Unknown Target',
    amount: product.price,
    reason: reason ?? 'Range Protocol: OFAC Match',
    status: 'pending',
    timestamp: Date.now()
  });
...
function init() {
  ...
  renderProducts();
  el.btnAddProduct?.addEventListener('click', (event) => {
    event.preventDefault();
    showListServiceModal();
  });
  el.listServiceCancel?.addEventListener('click', (event) => {
    event.preventDefault();
    hideListServiceModal();
  });
  el.listServiceForm?.addEventListener('submit', handleListServiceSubmit);
