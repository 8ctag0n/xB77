...
pub fn think(cli: *const Cli, args: []const [:0]u8) !void {
    if (args.len < 1) {
    ...
    var brain = core.intelligence.Brain.init(cli.allocator, &ctx.constitution);
    defer brain.deinit();

    var insight = try brain.reasonWithGemma(directive);
    defer insight.deinit();

    const report = try insight.formatTelegram();
