...
---
# Integración del Ecosistema y Alineación de Sponsors

::: warning Traducción Pendiente
Este documento aún está en Inglés. La traducción al Español estará disponible pronto.
:::
...
- **Status:** `FUNCTIONAL MVP (Actually using their rpcs)`
...
- **Status:** `INTEGRATION ROADMAP (it's integrated to the programs but I discovered a bug while doing the cpi, so im gonna fix it asap)`
...
- **Status:** `ARCHITECTURAL PROPOSAL (this would help a lot actually, but will be implemented in the next versions)`
