(function() {
  ...
  function computeParticle(p, cycle) {
    ...
    if (t < 0.55) {
      const a2 = easeInOut(t / 0.55);
      return {
        cx: lerp(sx, nx, a2),
        cy: lerp(sy, ny, a2),
        r: lerp(0.3, 0.85, a2),
        opacity: lerp(0, 0.85, Math.min(1, a2 * 1.6)),
        fill: COLORS.cyan
      };
    }
    if (t < 0.78) {
      const a2 = easeInOut((t - 0.55) / 0.23);
      return {
        cx: lerp(nx, CENTER.x, a2),
        cy: lerp(ny, CENTER.y, a2),
        r: lerp(0.85, 1.25, a2),
        opacity: lerp(0.85, 1, a2),
        fill: a2 > 0.5 ? COLORS.lime : COLORS.cyan
      };
    }
  ...
  function notchGlowVec(cycle) {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      const t = (cycle + i / PARTICLE_COUNT) % 1;
      if (t >= 0.5 && t <= 0.62) {
        const s = Math.sin((t - 0.5) / 0.12 * Math.PI);
  ...
  function emitWindow(cycle) {
    return cycle >= 0.78 && cycle <= 1 ? Math.sin((cycle - 0.78) / 0.22 * Math.PI) : 0;
  }
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(motion.rect, {
          ...
          animate: showBezel ? { strokeDashoffset: 0, opacity: 0.85 } : { strokeDashoffset: 232, opacity: 0 },
        ...
        React.createElement(
          motion.g,
          {
            ...
            animate: showMono ? { opacity: 1, y: 0, scale: [0.92, 1.05, 1] } : { opacity: 0, y: -6, scale: 0.92 },
        ...
        React.createElement(
          ...
          ticks.map(
            (x, i) => React.createElement(motion.line, {
              key: x,
              x1: x,
              y1: i === 0 ? 56.5 : i % 2 === 0 ? 57.5 : 58,
              x2: x,
              y2: 60.5,
              initial: { opacity: 0 },
        ...
        React.createElement(
          motion.g,
          {
            ...
            animate: showZK ? { opacity: 1, scale: [0.6, 1.05, 1], y: 0 } : { opacity: 0, scale: 0.6, y: -2 },
