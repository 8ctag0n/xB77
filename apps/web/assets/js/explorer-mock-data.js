...
function generatePipeline(id) {
  const now = Date.now();
  const ago = Math.floor(Math.random() * 864e5 * 7);
  return {
    ...
    amount: (Math.random() * 5e4 + 100).toFixed(2),
    currency: "USDC",
    ...
    fee: (Math.random() * 0.05 + 1e-3).toFixed(4),
    zkProof: generateHash(16),
    compressedState: generateHash(20),
    steps: [
      { label: "NEURAL_KEY_VERIFIED", status: "done", ts: now - ago },
      { label: "SHIELDING_ASSETS", status: "done", ts: now - ago + 2e3 },
      { label: "ZK_PROOF_GENERATED", status: "done", ts: now - ago + 5e3 },
      { label: "SETTLEMENT", status: Math.random() > 0.1 ? "done" : "pending", ts: now - ago + 8e3 }
    ]
  };
}
function generateZnode(id) {
  ...
  return {
    ...
    pipelines: Math.floor(Math.random() * 2e3 + 50),
    latency: Math.floor(Math.random() * 120 + 8),
    stake: (Math.random() * 1e5 + 1e3).toFixed(0),
...
function generateAgent(i) {
  ...
  return {
    ...
    volume: (Math.random() * 2e6 + 1e4).toFixed(0),
    lastActive: Date.now() - Math.floor(Math.random() * 36e5),
