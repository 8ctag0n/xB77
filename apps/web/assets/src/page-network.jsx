...
/* ── Section: Arc Swarm Intelligence (Circle Stack) ──────────────── */
function ArcPulseSection() {
  const [arc, setArc] = React.useState(null);
  React.useEffect(() => {
    if (!window.DataSource) return;
    return window.DataSource.subscribe('arcPulse', setArc, 3000);
  }, []);
  ...
  return (
    <section style={{ padding: '40px 0', borderBottom: `1px solid ${D.border}` }}>
      <div style={{
        ...
        <div>
          <DM>Arc Swarm Intelligence</DM>
          <div style={{ marginTop: 8 }}>
            <DS size={32} italic>Circle Agent Stack: USDC + USYC + CCTP.</DS>
          </div>
        </div>
        <NetStatusPill payload={arc} />
      </div>
