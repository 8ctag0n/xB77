/* xB77 Explorer v2 — Extended Mock Data
   Poseidon commits, APP, Merchants, Telegram, MCP
*/

// Poseidon commit types
...
function generatePoseidonCommit(i) {
  ...
  return {
    ...
    timestamp: now - Math.floor(Math.random() * 3600000 * 48),
    blockHeight: 280000000 + Math.floor(Math.random() * 1000000),
    zkProof: generateHash(12),
    gasUsed: Math.floor(Math.random() * 5000 + 200),
...
function generateMerchant(m, i) {
  return {
    ...
    totalVolume: (Math.random() * 500000 + 5000).toFixed(0),
    ...
    sparkVolume: Array.from({ length: 14 }, () => Math.random() * 1000 + 100),
  };
}
...
// Enhanced agents with richer data
function enhanceAgent(a, i) {
  return {
    ...
    totalEarnings: (Math.random() * 50000 + 500).toFixed(0),
  };
}
...
Object.assign(window, {
  MOCK_POSEIDON, MOCK_MERCHANTS, MOCK_AGENTS_V2,
  TELEGRAM_EVENTS, MCP_COMMANDS, POSEIDON_TYPES,
});
