...
test "Sovereign State: CMT Append and Verify" {
    ...
    var tree = try cmt.ConcurrentMerkleTree.init(allocator, 3);
    defer tree.deinit();
    ...
    // La prueba de leaf1 está en change_logs[1] (porque leaf2 es el más reciente en change_logs[0])
    const log1 = tree.change_logs.items[1];
    try std.testing.expect(log1.index == 0);
    try std.testing.expect(cmt.ConcurrentMerkleTree.verifyProof(root1, leaf1, 0, log1.siblings));
}
