...
/* ── Hash bootstrap ──────────────────────────────────────────────────
 * Takes over rendering when location.hash === '#network'. Hides the
 * original #root and mounts NetworkPage into its own container. On any
 * other hash, restores #root visibility and unmounts the network tree.
 * Does NOT modify router.jsx (W1-owned).
 */
(function NetworkHashBootstrap() {
  ...
  function ensureContainer() {
    ...
    container = document.getElementById('xb77-network-mount');
    if (!container) {
      container = document.createElement('div');
      container.id = 'xb77-network-mount';
  ...
  function handle() {
    ...
    if (!root) return;
    const wantNetwork = window.location.hash === '#network';

    if (wantNetwork && !active) {
      active = true;
      root.style.display = 'none';
      const c = ensureContainer();
      c.style.display = 'block';
      ReactDOM.render(React.createElement(window.NetworkPage), c);
      window.scrollTo(0, 0);
    } else if (!wantNetwork && active) {
      active = false;
      if (container) {
